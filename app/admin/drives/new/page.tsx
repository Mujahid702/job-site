...
export default function NewDrive() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    // Assuming 'drives' table or 'jobs' table with a 'type' field.
    // For now, we'll insert into a 'drives' table as requested.
    const { error } = await supabase.from("drives").insert([form]);

    if (error) {
      alert(error.message);
      setLoading(false);
    } else {
      router.push("/admin/drives"); // Assuming there's a manage drives page
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 uppercase tracking-widest">Drive Title</label>
                  <input name="title" onChange={handleChange} value={form.title} placeholder="Software Engineer Off-Campus Drive" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-slate-700 uppercase tracking-widest">Drive Slug (URL)</label>
                  <input name="slug" onChange={handleChange} value={form.slug} placeholder="ibm-off-campus-drive-2026" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>
                <div className="space-y-2">
            ...
            <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-8">
              ...
              {[
                { name: "description", label: "Drive Description" },
