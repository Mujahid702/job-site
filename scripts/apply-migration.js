...
async function applyMigrations() {
  ...
  const migrationPath = path.join(__dirname, "..", "supabase", "migrations", "012_production_subscriptions.sql");
  if (!fs.existsSync(migrationPath)) {
    console.error("❌ Migration file 012 not found.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log("✅ Migration successfully applied to remote database cache!");
    process.exit(0);
  } catch (err) {
